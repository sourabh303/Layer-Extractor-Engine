...
## 2025-05-31: Hardcoded Placeholder Secrets in Production Build
* **Vulnerability**: The .NET Orchestrator's `LicenseService` relied on hardcoded placeholder strings (`"placeholder-anon-key"` and `"https://placeholder-url.supabase.co"`) as fallbacks if the required Supabase environment variables were missing. If deployed or run without proper configuration, the application would silently start but fail in non-obvious ways, potentially logging sensitive requests to untrusted endpoints or bypassing expected security layers by allowing execution in a degraded/unconfigured state.
* **Learning**: Security configurations and credentials must never fail-open to hardcoded dummy values. A missing required credential is a critical configuration error.
* **Prevention**: Enforce a fail-fast mechanism. Throw an immediate, explicit `InvalidOperationException` (or similar) during application/service startup if required secure environment variables are missing, forcing the administrator or deployment pipeline to correctly provide the necessary secrets.
