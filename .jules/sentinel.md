...
## 2025-02-27 - [Supabase PostgREST API Injection in C#]
**Vulnerability:** The .NET orchestrator `LicenseService` extracted a `userId` string from a JWT and immediately interpolated it into a Supabase REST API URL string (`$"{_supabaseUrl}/rest/v1/licenses?user_id=eq.{userId}"`) without proper URL encoding. This allowed an attacker with a maliciously crafted JWT (if accepted by the auth system) to potentially inject arbitrary query parameters (like `&status=eq.active`) into the backend request.
**Learning:** URL interpolation vulnerabilities are not limited to SQL strings or frontend HTTP clients. Any server-to-server HTTP request constructing a URL dynamically using variables derived from user input or tokens must explicitly encode those variables to prevent REST API query pollution.
**Prevention:** To prevent PostgREST API Injection vulnerabilities in C# (.NET) when making REST API calls, always wrap dynamically inserted variables with `Uri.EscapeDataString()` before interpolating them into query strings.
